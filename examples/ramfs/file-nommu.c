...
static int cmpe142_nommu_setattr(struct dentry *, struct iattr *);
...
const struct file_operations cmpe142_file_operations = {
	.mmap			= cmpe142_nommu_mmap,
	.get_unmapped_area	= cmpe142_nommu_get_unmapped_area,
	.read			= do_sync_read,
 ...
 *
 */
static int cmpe142_nommu_resize(struct inode *inode, loff_t newsize, loff_t size)
{
	int ret;
	...
	if (size == 0) {
		...
		return cmpe142_nommu_expand_for_mapping(inode, newsize);
	}
 ...
 */
static int cmpe142_nommu_setattr(struct dentry *dentry, struct iattr *ia)
{
	...
	if (ia->ia_valid & ATTR_SIZE) {
		...
		if (ia->ia_size != size) {
			ret = cmpe142_nommu_resize(inode, ia->ia_size, size);
